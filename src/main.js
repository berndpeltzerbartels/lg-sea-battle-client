...
const observationPeriscopeHeadingMarker = document.getElementById("observationPeriscopeHeadingMarker");
...
const torpedoScopeZoomMarker = document.getElementById("torpedoScopeZoomMarker");
const torpedoScopeBearingValue = document.getElementById("torpedoScopeBearingValue");
const torpedoScopeTargetBearingValue = document.getElementById("torpedoScopeTargetBearingValue");
const torpedoScopeBearingMarker = document.getElementById("torpedoScopeBearingMarker");
...
const submarineObservationPeriscopeYawFineSpeed = 0.06;
const submarineObservationPeriscopeYawMediumSpeed = 0.24;
const submarineObservationPeriscopeYawFastSpeed = 0.42;
const submarineObservationPeriscopeYawVeryFastSpeed = 0.62;
const submarineObservationPeriscopeYawMaxSpeed = 0.78;
const submarineObservationPeriscopeYawExtremeSpeed = 0.92;
...
const submarineWaterlineY = torpedoBoatWaterlineY - 0.06 * submarineVisualScale;
...
const flakHoldMediumDelaySeconds = 0.25;
const flakHoldFastDelaySeconds = 0.65;
const flakHoldVeryFastDelaySeconds = 1.05;
const flakHoldMaxDelaySeconds = 1.45;
...
const flakPitchFineSpeed = flakYawFineSpeed;
const flakPitchMediumSpeed = flakYawMediumSpeed;
const flakPitchFastSpeed = flakYawFastSpeed;
const flakPitchVeryFastSpeed = flakYawVeryFastSpeed;
const flakPitchMaxSpeed = flakYawMaxSpeed;
const flakPitchExtremeSpeed = flakYawExtremeSpeed;
...
const submarineFlakRestPitch = Math.PI * 15 / 180;
...
const submarinePeriscopeZoomLevels = [
  { label: "I", fov: 0.64 },
  { label: "II", fov: 0.42 },
  { label: "III", fov: 0.28 },
  { label: "IV", fov: 0.18 },
...
const torpedoBoatScopeZoomLevels = [
  { label: "I", fov: 0.42 },
  { label: "II", fov: 0.28 },
  { label: "III", fov: 0.18 }
];
...
const botSubmarineMixRequested = urlParams.get("botSubmarines") === "1" || urlParams.get("bot-submarines") === "1";
...
window.addEventListener("keydown", (event) => {
  ...
  if (playerActive && submarineMode && event.shiftKey && isInputKey(event, "down") && !event.repeat) {
    stepPlayerSubmarineDepthState("down");
    event.preventDefault();
    return;
  }
  ...
  if (playerActive && isSubmarinePeriscopeModeKey(event) && !event.repeat) {
    setSubmarinePeriscopeMode(getSubmarinePeriscopeModeFromInput(event));
    event.preventDefault();
    return;
  }
  ...
  if (playerActive && isObservationPeriscopeZoomKey(event) && !event.repeat) {
    cycleObservationPeriscopeZoomLevel();
    event.preventDefault();
    return;
  }
  if (playerActive && isTorpedoScopeZoomKey(event) && !event.repeat) {
...
window.addEventListener("wheel", (event) => {
  ...
  if (isPlayerSubmarineObservationPeriscopeActive()) {
    updateObservationPeriscopeZoomFromWheel(event);
    event.preventDefault();
    return;
  }
  if (!scoutPlaneMode && torpedoScopeActive) {
...
let heldObservationPeriscopeYawStartTime = 0;
let heldObservationPeriscopePitchDirection = 0;
let submarinePeriscopeZoomLevelIndex = 0;
...
setupSubmarinePeriscopeModeControls();
...
scene.onBeforeRenderObservable.add(() => {
  ...
  if (playerActive && isPlayerSubmarineObservationPeriscopeActive()) {
    if (heldObservationPeriscopeYawDirection !== 0) {
      const observationPeriscopeYawSpeed = getHeldFlakSpeed(
        heldObservationPeriscopeYawStartTime,
        submarineObservationPeriscopeYawFineSpeed,
        submarineObservationPeriscopeYawMediumSpeed,
        submarineObservationPeriscopeYawFastSpeed,
        submarineObservationPeriscopeYawVeryFastSpeed,
        submarineObservationPeriscopeYawMaxSpeed,
        submarineObservationPeriscopeYawExtremeSpeed
      );
      observationPeriscopeYaw = normalizeAngle(
        observationPeriscopeYaw + heldObservationPeriscopeYawDirection * observationPeriscopeYawSpeed * dt
      );
    }
    if (heldObservationPeriscopePitchDirection !== 0) {
      observationPeriscopePitch = clamp(
        observationPeriscopePitch + heldObservationPeriscopePitchDirection * submarineObservationPeriscopePitchSpeed * dt,
        submarineObservationPeriscopePitchMin,
        submarineObservationPeriscopePitchMax
      );
    }
  }
...
function isObservationPeriscopeZoomKey(event) {
  return isPlayerSubmarineObservationPeriscopeActive() && (event.code === "KeyZ" || event.key === "z" || event.key === "Z");
}

function isTorpedoScopeZoomKey(event) {
  return torpedoScopeActive && !scoutPlaneMode && (event.code === "KeyZ" || event.key === "z" || event.key === "Z");
}

function isSubmarinePeriscopeModeKey(event) {
  return submarineMode
    && canShowSubmarineTorpedoScope()
...
function updateTorpedoViewState() {
  ...
  updateTorpedoScopeBearingDisplay();
  updateTorpedoScopeRudderDisplay();
  updateSubmarinePeriscopeModeUi();
}
...
function startSubmarineBearingAlignment() {
  if (!submarineMode || !canUseSubmarineTorpedoScope()) return;
  submarineBearingAlignTarget = normalizeAngle(heading + observationPeriscopeYaw);
...
function getCurrentSubmarinePeriscopeBearing() {
  if (isPlayerSubmarineObservationPeriscopeActive()) {
    return normalizeAngle(heading + observationPeriscopeYaw);
  }
...
function updateSubmarineBearingAlignment(dt) {
  ...
  const speedDirection = speed < -submarineBearingAlignStopSpeed ? -1 : 1;
  const rudderSign = Math.sign(delta) * speedDirection;
...
function updateSubmarinePeriscopeModeUi() {
  if (!submarineMode) return;
...
function canUseTorpedoScope() {
  return !scoutPlaneMode
    ...
    && (!submarineMode || canShowSubmarineTorpedoScope());
}

function canShowSubmarineTorpedoScope() {
  return !submarineMode
    || submarineBridgeAscentHoldActive
    || playerSubmarineDepthState === submarineDepthStates.periscope
...
function getSubmarineBridgeCameraLocalPosition() {
  return {
    position: new Vector3(0, 1.58, 0.23),
...
function getSelectedRadarRange() {
  const effectiveSubmarineDepthState = getPlayerEffectiveSubmarineDepthState();
  if (submarineMode && effectiveSubmarineDepthState === submarineDepthStates.submerged) {
    document.body.dataset.radarDepthMode = "submerged";
    return clientRadarRange * combatRadarRangeFactor * submarineSubmergedRadarRangeFactor;
  }
  if (singleRadarMode) {
...
function configureVehicleHud() {
  ...
  if (torpedoAidButton) {
    ...
    torpedoAidButton.setAttribute("aria-label", "Torpedo wählen");
    const label = torpedoAidButton.querySelector("span");
    if (label) {
      label.textContent = "Torpedo";
    }
  }
}
...
function setupSubmarineDepthControl(button) {
  ...
  button.addEventListener("click", (event) => {
    ...
    if (depthState === submarineDepthStates.surface) {
      stepPlayerSubmarineDepthState("up");
    } else if (depthState === submarineDepthStates.submerged) {
      stepPlayerSubmarineDepthState("down");
    } else if (depthState === submarineDepthStates.periscope) {
      setPlayerSubmarineDepthState(submarineDepthStates.periscope);
    }
    button.blur();
    event.stopPropagation();
...
function setupSubmarinePeriscopeModeControls() {
  submarinePeriscopeModeButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      setSubmarinePeriscopeMode(button.dataset.submarinePeriscopeMode);
      button.blur();
      event.stopPropagation();
    });
  });
...
function getTorpedoScopeFov() {
  return currentTorpedoScopeZoomLevel().fov;
}

function getObservationPeriscopeFov() {
  return currentSubmarineObservationPeriscopeZoomLevel().fov;
}
...
function currentSubmarineTorpedoScopeZoomLevel() {
  return currentSubmarinePeriscopeZoomLevel();
}

function currentSubmarineObservationPeriscopeZoomLevel() {
  return currentSubmarinePeriscopeZoomLevel();
}

function currentTorpedoBoatScopeZoomLevel() {
  return torpedoBoatScopeZoomLevels[torpedoBoatScopeZoomLevelIndex] ?? torpedoBoatScopeZoomLevels[0];
}

function currentTorpedoScopeZoomLevel() {
  return submarineMode ? currentSubmarineTorpedoScopeZoomLevel() : currentTorpedoBoatScopeZoomLevel();
}

function currentSubmarinePeriscopeZoomLevel() {
  return submarinePeriscopeZoomLevels[submarinePeriscopeZoomLevelIndex] ?? submarinePeriscopeZoomLevels[0];
}

function cycleTorpedoScopeZoomLevel() {
  if (submarineMode) {
    cycleSubmarinePeriscopeZoomLevel();
    return;
  }
  torpedoBoatScopeZoomLevelIndex = nextTorpedoBoatScopeZoomIndex(
    torpedoBoatScopeZoomLevelIndex,
    torpedoBoatScopeZoomLevels.length
  );
  updateTorpedoScopeZoomDisplay();
}
...
function cycleSubmarinePeriscopeZoomLevel() {
  submarinePeriscopeZoomLevelIndex = nextPingPongZoomIndex(
    submarinePeriscopeZoomLevelIndex,
    submarinePeriscopeZoomLevels.length
  );
  updateSubmarinePeriscopeZoomDisplays();
}

function nextPingPongZoomIndex(index, levelCount) {
  if (levelCount <= 1) return 0;
  let direction = submarinePeriscopeZoomCycleDirection;
  ...
  submarinePeriscopeZoomCycleDirection = direction;
  return boundedIndex;
}

function nextTorpedoBoatScopeZoomIndex(index, levelCount) {
  if (levelCount <= 1) return 0;
  let direction = torpedoBoatScopeZoomCycleDirection;
  const nextIndex = index + direction;
  if (nextIndex >= levelCount) {
    direction = -1;
  } else if (nextIndex < 0) {
  ...
  torpedoBoatScopeZoomCycleDirection = direction;
  return boundedIndex;
}
...
function changeTorpedoScopeZoomLevel(direction) {
  if (submarineMode) {
    changeSubmarinePeriscopeZoomLevel(direction);
    return;
  }
  torpedoBoatScopeZoomLevelIndex = clamp(torpedoBoatScopeZoomLevelIndex + direction, 0, torpedoBoatScopeZoomLevels.length - 1);
  torpedoBoatScopeZoomCycleDirection = direction >= 0 ? 1 : -1;
  updateTorpedoScopeZoomDisplay();
}
...
function updateObservationPeriscopeZoomFromWheel(event) {
  mouseWheelCannonSightAccumulator += event.deltaY;
  while (mouseWheelCannonSightAccumulator <= -mouseWheelEngineStep) {
    changeObservationPeriscopeZoomLevel(1);
    mouseWheelCannonSightAccumulator += mouseWheelEngineStep;
  }
  while (mouseWheelCannonSightAccumulator >= mouseWheelEngineStep) {
    changeObservationPeriscopeZoomLevel(-1);
...
function changeSubmarinePeriscopeZoomLevel(direction) {
  const nextIndex = clamp(submarinePeriscopeZoomLevelIndex + direction, 0, submarinePeriscopeZoomLevels.length - 1);
  submarinePeriscopeZoomLevelIndex = nextIndex;
  submarinePeriscopeZoomCycleDirection = direction >= 0 ? 1 : -1;
  updateSubmarinePeriscopeZoomDisplays();
}

function updateSubmarinePeriscopeZoomDisplays() {
  updateTorpedoScopeZoomDisplay();
  updateObservationPeriscopeZoomDisplay();
}

function updateTorpedoScopeZoomDisplay() {
  const label = currentTorpedoScopeZoomLevel().label;
  document.body.dataset.torpedoScopeZoom = label;
  if (torpedoScopeZoomValue) {
    torpedoScopeZoomValue.textContent = label;
  }
  if (torpedoScopeZoomMarker) {
    const levels = submarineMode ? submarinePeriscopeZoomLevels : torpedoBoatScopeZoomLevels;
    const index = submarineMode ? submarinePeriscopeZoomLevelIndex : torpedoBoatScopeZoomLevelIndex;
    const maxIndex = Math.max(1, levels.length - 1);
    const ratio = index / maxIndex;
    torpedoScopeZoomMarker.style.top = `${100 - ratio * 100}%`;
  }
}

function updateTorpedoScopeRudderDisplay() {
  if (!torpedoScopeRudderMarker) return;
  torpedoScopeRudderMarker.style.left = `${rudderScaleLeft(rudderDegrees)}%`;
}
...
function updateObservationPeriscopeZoomDisplay() {
  ...
  if (observationPeriscopeZoomMarker) {
    ...
    observationPeriscopeZoomMarker.style.top = `${100 - ratio * 100}%`;
  }
}
...
function pressDirectionalInput(direction, options = {}) {
  ...
  if (direction === "left") {
    if (isPlayerSubmarineObservationPeriscopeActive()) {
      if (!repeat || heldObservationPeriscopeYawDirection !== -1) {
        heldObservationPeriscopeYawStartTime = time;
      }
...
function releaseDirectionalInput(direction) {
  if (direction === "up") {
    ...
    if (heldObservationPeriscopeYawDirection < 0) heldObservationPeriscopeYawDirection = 0;
...
function stepPlayerSubmarineDepthState(direction) {
  const depthOrder = [
    submarineDepthStates.surface,
    submarineDepthStates.periscope,
    submarineDepthStates.submerged
  ];
  const currentIndex = depthOrder.indexOf(playerSubmarineDepthState);
  const step = direction === "up" ? -1 : 1;
  const nextIndex = clamp(currentIndex + step, 0, depthOrder.length - 1);
  setPlayerSubmarineDepthState(depthOrder[nextIndex] ?? submarineDepthStates.surface);
}
...
function setPlayerSubmarineDepthState(depthState) {
  ...
  submarineBridgeAscentHoldActive = previousDepthState !== submarineDepthStates.surface
...
function getPlayerEffectiveSubmarineDepthState() {
  if (!submarineMode) return submarineDepthStates.surface;
  const depth = Math.abs(playerSubmarineDepthOffset);
  if (depth >= Math.abs(submarineDepthOffsets.submerged) - 0.08) {
    return submarineDepthStates.submerged;
  }
  if (depth >= Math.abs(submarineDepthOffsets.periscope) - 0.08) {
    return submarineDepthStates.periscope;
  }
...
function updateObservationPeriscopeHeadingDisplay(active = isPlayerSubmarineObservationPeriscopeActive()) {
  ...
  if (observationPeriscopeBearingValue) {
    observationPeriscopeBearingValue.textContent = `${formatHeadingDegrees(absoluteBearing)}°`;
  }
  if (observationPeriscopeHeadingNeedle) {
    observationPeriscopeHeadingNeedle.style.transform = `rotate(${observationPeriscopeYaw}rad)`;
  }
  if (observationPeriscopeYawMarker) {
    observationPeriscopeYawMarker.style.left = `${signedAngleScaleLeft(observationPeriscopeYaw)}%`;
  }
  updateSubmarinePeriscopeModeUi();
}
...
function updateTorpedoScopeBearingDisplay() {
  const bearingRatio = compassBearingRatio(heading);
  const targetBearingRatio = compassBearingRatio(submarineBearingAlignTarget);
  if (torpedoScopeBearingMarker) {
    torpedoScopeBearingMarker.style.top = `${clamp(bearingRatio * 100, 0, 100)}%`;
  }
  if (torpedoScopeTargetBearingMarker) {
...
function compassBearingRatio(headingValue) {
  const degrees = ((headingValue * 180 / Math.PI) % 360 + 360) % 360;
  return degrees / 360;
}
...
function updateSubmarineDepthUi() {
  if (!submarineMode) return;
  if (submarineSurfaceButton) {
    submarineSurfaceButton.disabled = playerSubmarineDepthState === submarineDepthStates.surface;
  }
  if (submarineDiveButton) {
    submarineDiveButton.disabled = playerSubmarineDepthState === submarineDepthStates.submerged;
  }
}
...
function indexCriticalFlakShipHitsByTarget(hits) {
...
function drawRadarInstrument(canvas, statusElement, playerPosition, radarContacts, landZones, heading, range = 360, options = {}) {
  ...
  const radarDepthMode = document.body.dataset.radarDepthMode ?? "normal";
  const monochromeMode = radarDepthMode === "submerged";
  ...
  ctx.clip();
  ctx.fillStyle = monochromeMode ? "rgba(0, 4, 9, 0.96)" : "rgba(2, 22, 28, 0.86)";
  ctx.fillRect(0, 0, width, height);
  ...
  drawRadarRangeRings(ctx, centerX, centerY, radius, monochromeMode);
  drawRadarLandUnion(ctx, landZones, playerPosition, centerX, centerY, scale, heading, width, height, monochromeMode);
  ...
  if (nearestVisible) {
    ...
    if (statusElement) statusElement.textContent = monochromeMode
      ? `Echo ${formatWorldDistance(nearestVisible.distance)}${suffix}`
      : `${label} ${formatWorldDistance(nearestVisible.distance)}${suffix}`;
  } else {
    if (statusElement) statusElement.textContent = monochromeMode
      ? `Tiefenscan ${formatWorldDistance(radarRange)}`
      : `Clear ${formatWorldDistance(radarRange)}`;
  }

  drawRadarContactMarker(ctx, centerX, centerY, "light", true, null, heading, "", playerVehicleType, contactMarkerScale, monochromeMode);
  ...
  ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
  ctx.stroke();
  drawRadarCompassRing(ctx, centerX, centerY, radius, heading, monochromeMode);
}
...
function drawRadarContactMarker(ctx, x, y, team, isPlayer = false, contactHeading = null, radarHeading = 0, label = "", vehicleType = "torpedo-boat", markerScale = 1, monochromeMode = false) {
  ...
  if (isPlayer && monochromeMode && vehicleType === "submarine") {
    drawRadarSubmergedOwnSubmarineMarker(ctx, x, y, color, 0, scaledMarker);
    return;
  }
  ...
  if (!isPlayer && label) {
    ...
    ctx.strokeStyle = monochromeMode ? "rgba(0, 5, 10, 0.94)" : "rgba(2, 16, 21, 0.92)";
...
function drawRadarSubmergedOwnSubmarineMarker(ctx, x, y, color, relativeHeading, markerScale = 1) {
  ...
  ctx.save();
  ctx.fillStyle = color;
  ctx.strokeStyle = "rgba(0, 5, 10, 0.92)";
  ctx.lineWidth = clamp(1.2 * markerScale, 0.8, 1.35);
  ctx.beginPath();
  moveToRadarMarkerPoint(ctx, toPoint, 0, 8.8);
  lineToRadarMarkerPoint(ctx, toPoint, 2.1, 4.8);
  lineToRadarMarkerPoint(ctx, toPoint, 2.4, -5.8);
  lineToRadarMarkerPoint(ctx, toPoint, 0, -8.8);
  lineToRadarMarkerPoint(ctx, toPoint, -2.4, -5.8);
  lineToRadarMarkerPoint(ctx, toPoint, -2.1, 4.8);
  ctx.closePath();
  ctx.stroke();
  ...
  ctx.strokeStyle = "rgba(214, 248, 255, 0.58)";
  ctx.lineWidth = clamp(0.9 * markerScale, 0.65, 1.1);
  ctx.beginPath();
  moveToRadarMarkerPoint(ctx, toPoint, 0, 8.8);
  lineToRadarMarkerPoint(ctx, toPoint, 0, -8.8);
  ctx.stroke();
...
function drawRadarCompassRing(ctx, centerX, centerY, radius, radarHeading, monochromeMode = false) {
  const labels = [
    ["N", 0],
    ["E", Math.PI / 2],
    ["S", Math.PI],
...
function drawRadarOwnHeadingMarker(ctx, centerX, centerY, monochromeMode = false) {
  ctx.save();
  ctx.fillStyle = monochromeMode ? "rgba(214, 248, 255, 0.9)" : "rgba(247, 251, 255, 0.94)";
  ctx.strokeStyle = monochromeMode ? "rgba(0, 5, 10, 0.92)" : "rgba(2, 16, 21, 0.86)";
  ctx.lineWidth = 1;
  ctx.beginPath();
...
function drawRadarRangeRings(ctx, centerX, centerY, radius, monochromeMode = false) {
  if (!Number.isFinite(radius) || radius <= 0) return;
  ctx.strokeStyle = monochromeMode ? "rgba(214, 248, 255, 0.18)" : "rgba(155, 229, 223, 0.22)";
  ctx.lineWidth = 1;
...
function updateEnemyShipCriticalHit(motion, dt, now) {
  ...
  if (now >= (motion.criticalHitNextSmokeTime ?? now)) {
    createBurningDamageTrail(flakSystem, Vector3.Zero(), new Vector3(0, 0.28, 0), motion.criticalHitAnchor);
...
function respawnPlayerBoat(playerBoat) {
  ...
  if (pendingPlayerServerShip) {
    ...
    playerDamageState = "active";
    resetPlayerSubmarineDepthAfterRespawn(playerBoat);
...
function resetPlayerSubmarineDepthAfterRespawn(playerBoat) {
  if (!submarineMode) return;
  playerSubmarineDepthOffset = submarineDepthOffsets.surface;
...
function installScenarioTestHooks() {
  ...
  window.seaBattleScenarioTest = {
    ...
    respawnPlayerForTest(depthState = submarineDepthStates.surface) {
      if (submarineMode) {
        setPlayerSubmarineDepthState(depthState);
        playerSubmarineDepthOffset = submarineDepthOffsets[sanitizeSubmarineDepthState(depthState)] ?? 0;
        updatePlayerSubmarineDiveMotion(0);
      }
      playerDamageState = "sinking";
      respawnPlayerBoat(boat);
      return this.submarineDiveSequenceSnapshot();
    },
    submarineDiveSequenceSnapshot() {
      ...
      return {
        ...
        radarTargetLineMode: document.body.dataset.radarTargetLineMode ?? "hidden",
        ...
        observationPeriscopeZoom: document.body.dataset.observationPeriscopeZoom ?? "I",
    ...
    cannonShotLineAt(angles) {
      ...
      return {
        ...
        blocker: document.body.dataset.ownShotBlocker ?? "",
        shot: shot
          ? {
    ...
    flakShotLineAt(angles) {
      setBattleStation("flak");
      flakYaw = normalizeAngle(Number(angles?.yaw ?? 0));
      flakPitch = clamp(Number(angles?.pitch ?? 0), flakMinPitch, flakMaxPitch);
      updatePlayerFlakMount();
      const shot = getPlayerFlakShot();
      return {
        yaw: flakYaw,
        pitch: flakPitch,
        blocked: flakShotWouldHitOwnBoat(shot),
        blocker: document.body.dataset.ownShotBlocker ?? "",
        shot: shot
          ? {
            position: { x: shot.position.x, y: shot.position.y, z: shot.position.z },
    ...
    cannonShotLineAtLocalTarget(target) {
      const scale = Number(target?.scale ?? torpedoBoatVisualScale);
      const localTarget = new Vector3(
        Number(target?.x ?? 0),
        Number(target?.y ?? 0),
        Number(target?.z ?? 0)
      ).scale(scale);
      const worldTarget = boat.root.position.add(
        Vector3.TransformCoordinates(localTarget, Matrix.RotationY(heading))
      );
      const aim = aimPlayerCannonAtWorldPoint(worldTarget);
      const shot = getPlayerCannonShot();
      return {
        aim,
        blocked: cannonShotWouldHitOwnBoat(shot),
    ...
    flakShotLineAtLocalTarget(target) {
      const scale = Number(target?.scale ?? torpedoBoatVisualScale);
      const localTarget = new Vector3(
        Number(target?.x ?? 0),
        Number(target?.y ?? 0),
      ...
      const aim = aimPlayerFlakAtWorldPoint(worldTarget);
      const shot = getPlayerFlakShot();
      return {
        aim,
        blocked: flakShotWouldHitOwnBoat(shot),
    ...
    submarinePeriscopeMaterialsForTest() {
      return Array.from(boat?.periscopeMasts ?? [])
        .map((mast) => mast?.mesh)
        .filter(Boolean)
        .map((mesh) => ({
          name: mesh.name,
    ...
    vehicleVisual(vehicleId) {
      ...
      const rotation = motion.root.rotationQuaternion?.toEulerAngles?.();
      return {
        id: motion.id,
...
function getSubmarineDiveSequenceCameraSetupForTest() {
  ...
  if (submarineMode && isPlayerSubmarineObservationPeriscopeActive()) {
    ...
    return {
      position,
      target: position.add(new Vector3(
        Math.sin(viewYaw) * Math.cos(viewPitch),
        Math.sin(viewPitch),
        Math.cos(viewYaw) * Math.cos(viewPitch)
      ).scale(88 * submarineVisualScale))
...
function flakShotWouldHitOwnBoat(shot) {
  return shotWouldHitOwnBoat(shot, 0.12, 2.2, "flak-periscope")
    || shotWouldHitOwnBoat(shot, 0.72, 19, "flak");
}
...
function shotWouldHitOwnBoat(shot, startDistance, length, weapon) {
  ...
  const safetyRadius = ownShotSafetyRadius(weapon);
  const blocker = blockers.find((mesh) => shotTubeIntersectsMesh(origin, direction, length, mesh, safetyRadius));
  document.body.dataset.ownShotBlocker = blocker?.name ?? "";
  return Boolean(blocker);
}

function ownShotSafetyRadius(weapon) {
  return 0.02;
}
...
function isOwnBoatShotBlocker(mesh, weapon = "") {
  ...
  if (weapon === "flak-periscope") return name.includes("_periscope_");
...
function shotTubeIntersectsMesh(origin, direction, length, mesh, radius = 0) {
  return shotTubeRayOrigins(origin, direction, radius)
    .some((rayOrigin) => shotRayIntersectsMesh(rayOrigin, direction, length, mesh));
}

function shotTubeRayOrigins(origin, direction, radius) {
  if (!Number.isFinite(radius) || radius <= 0.001) return [origin];
  let side = Vector3.Cross(Vector3.Up(), direction);
  if (side.lengthSquared() <= 0.0001) {
    side = Vector3.Cross(Vector3.Right(), direction);
  }
  side.normalize();
  const vertical = Vector3.Cross(direction, side).normalize();
  return [
    origin,
    origin.add(side.scale(radius)),
    origin.add(side.scale(-radius)),
    origin.add(vertical.scale(radius)),
    origin.add(vertical.scale(-radius))
  ];
}

function shotRayIntersectsMesh(origin, direction, length, mesh) {
  if (typeof mesh.computeWorldMatrix === "function") {
    mesh.computeWorldMatrix(true);
  }
  const pick = new Ray(origin, direction, length).intersectsMesh(mesh, false, null, false, undefined, false);
  return Boolean(pick?.hit && Number.isFinite(pick.distance) && pick.distance >= 0 && pick.distance <= length);
}
...
function createShipImpactSpark(system, position, diameter = 0.42, lifetime = 0.22, parent = null) {
  ...
  spark.parent = parent ?? system.root;
...
function createShipSuperstructureFire(system, position, count, scale = 1, parent = null) {
  for (let index = 0; index < count; index += 1) {
    ...
    flame.parent = parent ?? system.root;
...
function createBurningDamageTrail(system, position, drift = Vector3.Zero(), parent = null) {
  createDamageSmokePuff(system, position.add(drift), parent);
...
function createDamageSmokePuff(system, position, parent = null) {
